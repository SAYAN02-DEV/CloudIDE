...
exports.getSQSTerminalService = exports.SQSTerminalService = void 0;
const client_sqs_1 = require("@aws-sdk/client-sqs");
const redis_1 = require("redis");
class SQSTerminalService {
    constructor() {
            ...
            try {
                ...
                if (response.Messages && response.Messages.length > 0) {
                    for (const message of response.Messages) {
                        try {
                            const commandMessage = JSON.parse(message.Body || '{}');
                            // Process the command
     ...
     */
    async saveTerminalState(projectId, terminalId, state) {
     ...
     */
    async getTerminalState(projectId, terminalId) {
        const key = `terminal:state:${projectId}:${terminalId}`;
     ...
     */
    async registerTerminalSession(projectId, terminalId, workerId) {
     ...
     */
    async unregisterTerminalSession(projectId, terminalId) {
        const key = `terminal:session:${projectId}:${terminalId}`;
     ...
     */
    async isTerminalSessionActive(projectId, terminalId) {
        const key = `terminal:session:${projectId}:${terminalId}`;
...
exports.SQSTerminalService = SQSTerminalService;
// Singleton
let sqsTerminalService = null;
const getSQSTerminalService = () => {
    if (!sqsTerminalService) {
        sqsTerminalService = new SQSTerminalService();
    }
    return sqsTerminalService;
};
exports.getSQSTerminalService = getSQSTerminalService;
